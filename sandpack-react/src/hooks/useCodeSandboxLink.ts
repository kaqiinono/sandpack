...
const getFileParameters = (files: SandpackBundlerFiles, environment?: SandboxEnvironment) => {
    const normalized: Record<string, { content: string; isBinary: boolean }> = Object.keys(
        files
    ).reduce(
        (prev, next) => ({
            ...prev,
